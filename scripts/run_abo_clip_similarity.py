...
def run_clip_similarity(products: Sequence[ABOCLIPProduct], product_records: Sequence[Mapping[str, Any]], input_path: Path, query_item_id: str | None = None, top_k: int = 5, model_name: str = DEFAULT_CLIP_MODEL_NAME) -> dict[str, Any]:
    ...
    results = ABOCLIPSimilarityModel(model_name=model_name).similar_items(products, selected_query, top_k)
    return {
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="abo-clip-run-") as temp_dir:
        ...
        output = run_clip_similarity(build_clip_products(records, image_root), records, args.input, args.query_item_id, args.top_k, args.model_name)
